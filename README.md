...
- **JSON** ---> The format in which the webpage's datas are stored
- **Vite.dev** ---> The starting dev tool for this project
- **Netlify** ---> Where the project is deployed
...
**🟢 Live Deployment:** View the live app on Netlify : (https://ph-b14-a5-the-devstack.netlify.app/)
...
- **Parent to Child:** I can pass data from a parent component to a child component through `props`.
- **Child to Parent:** As per my knowledge, react is a uni-directional system where a child component cannot actually send something to a parent component. It can pass arguements to the funtion or props sent by the parent to call or use them. Which we can indirectly say passing that arguement data to the parent class as it is written or initiated in it.
